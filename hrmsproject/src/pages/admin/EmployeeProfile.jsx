...
export default function EmployeeProfile() {
  ...
  useEffect(() => {
    const fetchEmp = async () => {
      ...
      setError("");
      try {
        const res = await fetch(`http://localhost:8080/api/employees/${id}`, { credentials: "include" });
        if (!res.ok) {
  ...
  useEffect(() => {
    const fetchLeaveBalance = async () => {
      if (activeSection === 'leave_balance' && id && !leaveBalance) {
        ...
        try {
          const res = await fetch(`http://localhost:8080/api/leaves/balance/${id}`, { credentials: "include" });
          if (res.ok) {
            const json = await res.json();
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-brand-blue/60 font-medium">Manage candidate details and documents</p>
            </div>
                        ...
                        {[
                          ...
                          <div key={field.name} className="space-y-2">
                            ...
                            <input
                              ...
                              className={`w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all ${(!editing || field.disabled) ? 'opacity-60 cursor-not-allowed' : ''}`}
                            />
                          </div>
                        ))}
                        <div className="space-y-2">
                          <label className="text-xs font-bold text-brand-blue/40 uppercase tracking-widest ml-1">Current Address</label>
                          <textarea name="currentAddress" value={form.currentAddress || ''} onChange={handleChange} disabled={!editing} rows="2" className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all" />
                        </div>
                        <div className="space-y-2">
                          <label className="text-xs font-bold text-brand-blue/40 uppercase tracking-widest ml-1">Permanent Address</label>
                          <textarea name="permanentAddress" value={form.permanentAddress || ''} onChange={handleChange} disabled={!editing} rows="2" className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all" />
                        </div>
                      </div>
                    </div>
                  )}
                      ...
                      {[
                        ...
                        <div key={field.name} className="space-y-2">
                          ...
                          <input
                            ...
                            onChange={handleChange}
                            disabled={!editing}
                            className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all"
                          />
                        </div>
                      ))}
                      <div className="md:col-span-2 space-y-2">
                        <label className="text-xs font-bold text-brand-blue/40 uppercase tracking-widest ml-1">Address</label>
                        <textarea name="emergencyAddress" value={form.emergencyAddress || ''} onChange={handleChange} disabled={!editing} rows="3" className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-brand-blue focus:ring-2 focus:ring-brand-yellow/50 transition-all" />
                      </div>
                    </div>
                  )}
                        ...
                        <div key={cat} className="bg-white rounded-2xl p-6 border border-brand-blue/5 shadow-sm">
                          <div className="flex justify-between items-center mb-6">
                            <div>
                              ...
                              <p className="text-xs text-brand-blue/40 font-medium whitespace-nowrap overflow-hidden text-ellipsis">Manage your {cat} certificates</p>
                            </div>
                            {editing && (
                      ...
                      {fetchingBalance ? (
                        ...
                        <div className="bg-gray-50 rounded-2xl p-12 text-center border border-dashed border-brand-blue/10">
                          ...
                          <button
                            onClick={async () => {
                              try {
                                const res = await fetch(`http://localhost:8080/api/leaves/balance/initialize/${id}`, {
                                  method: 'POST',
                                  credentials: "include"
                                });
                                if (res.ok) {
